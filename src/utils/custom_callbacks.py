...
from src.utils.custom_evaluation import evaluate_policy_meta_lunarlander as custom_evaluate_policy_meta_lunarlander
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            (episode_rewards, episode_lengths,
             ...
             number_of_wins) = custom_evaluate_policy_meta_lunarlander(self.model,
                self.eval_env,
                n_eval_episodes=self.n_eval_episodes,
                render=self.render,
                deterministic=self.deterministic,
            ...
            if self.log_path is not None:
                ...
                np.savez(
                    ...
                    results=self.evaluations_results,
                    ep_lengths=self.evaluations_length,
                    agent_one_reward=mean_episode_agent_one_reward,
            ...
            mean_agent_one_reward, std_agent_one_reward = (np.mean(mean_episode_agent_one_reward),
                                                           np.std(mean_episode_agent_one_reward))
            mean_agent_two_reward, std_agent_two_reward = (np.mean(mean_episode_agent_two_reward),
                                                           np.std(mean_episode_agent_two_reward))
            self.last_mean_reward = mean_reward
            ...
            if self.verbose > 0:
                print(
                    f"Eval num_timesteps={self.num_timesteps}," f"episode_reward={mean_reward:.2f} +/- {std_reward:.2f}")
                print(f"Episode length: {mean_ep_length:.2f} +/- {std_ep_length:.2f}")
                print(f"Agent One Reward: {mean_agent_one_reward:.2f} +/- {std_agent_one_reward:.2f}")
                print(f"Agent Two Reward: {mean_agent_two_reward:.2f} +/- {std_agent_two_reward:.2f}")
